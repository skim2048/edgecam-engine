...
def validate_image_size(width: int, height: int) -> None:
    if width <= 0 or height <= 0:
        raise ValueError(f"Image size must be positive, but got width={width}, height={height}.")


def validate_image_format(array: np.ndarray) -> None:
    if not isinstance(array, np.ndarray):
        raise TypeError(f"Expected array to be a numpy.ndarray, but got {type(array).__name__}.")
    if array.ndim != 3:
        raise ValueError(f"Expected image array to have 3 dimensions, but got {array.ndim}.")

    height, width, channels = array.shape
    if channels != 3:
        raise ValueError(f"Expected 3 color channels, but got {channels}. Please verify the image layout (HWC vs. CHW).")
    ...
    if ensure_valid_inputs:
        for size in kernel_size:
            if size < 3 or size % 2 != 1:
                raise ValueError("Each element of the kernel size must be an odd number greater than 1.")
...
def pixelate_box_regions(image: np.ndarray, boxes: np.ndarray, pixel_size: int=5, ensure_valid_inputs: bool=False) -> None:
    if ensure_valid_inputs:
        if pixel_size <= 1:
            raise ValueError("The pixel size must be greater than 1.")
        validate_image_format(image)
        validate_boxes_format(boxes)
